import { MushroomImage } from "@assets/images";

const postCardsData = [
  {
    id: 1,
    profileImage: MushroomImage,
    nickname: "너무 착한 버섯",
    createdAt: "2025년 2월 8일 15:20",
    title: "프론트엔드 관련 프로젝트",
    content: "프론트엔드 개발을 같이 진행할 멤버를 찾습니다.프론트엔드 개발을 같이 진행할 멤버를 찾습니다.프론트엔드 개발을 같이 진행할 멤버를 찾습니다.프론트엔드 개발을 같이 진행할 멤버를 찾습니다.프론트엔드 개발을 같이 진행할 멤버를 찾습니다.프론트엔드 개발을 같이 진행할 멤버를 찾습니다.프론트엔드 개발을 같이 진행할 멤버를 찾습니다.프론트엔드 개발을 같이 진행할 멤버를 찾습니다.",
    likeCount: 8,
    categories: ["프론트엔드"],
  },
  {
    id: 2,
    profileImage: MushroomImage,
    nickname: "친절한 디자이너",
    createdAt: "2025년 2월 9일 10:00",
    title: "디자인 컨셉 논의",
    content: "UI/UX 디자인에 관심 있는 분들과 협업하고 싶습니다.",
    likeCount: 12,
    categories: ["디자인"],
  },
  {
    id: 3,
    profileImage: MushroomImage,
    nickname: "열정적인 백엔드 개발자",
    createdAt: "2025년 2월 10일 18:00",
    title: "백엔드 서버 구축",
    content: "백엔드 서버를 구축할 팀원을 모집합니다.",
    likeCount: 5,
    categories: ["백엔드"],
  },
  {
    id: 4,
    profileImage: MushroomImage,
    nickname: "기획 천재",
    createdAt: "2025년 2월 11일 09:30",
    title: "기획 단계 협업",
    content: "서비스 기획에 함께할 분을 찾고 있습니다.",
    likeCount: 3,
    categories: ["기획"],
  },
];

export default postCardsData;
