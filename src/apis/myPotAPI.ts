...
export const patchMyPotTodoStatus = async ({
  potId,
  todoId,
}: PatchTodoStatusParams) => {
  return authApiPatch(`/my-pots/${potId}/todos/${todoId}`, {
    status: "COMPLETED",
  });
};
...
export const patchMyPotTask = async (
  { potId, taskId }: TaskAPIParams,
  data: TaskPatch
) => {
...
export const postMyPotTask = async ({
  potId,
  data,
}: {
  potId: number;
...
export const patchMyPotStatus = async (
  { potId, taskId }: TaskAPIParams,
  data: PatchStatus
) => {
  return authApiPatch(
    `/my-pots/${potId}/tasks/${taskId}/status?status=${data.status}`
  );
};
...
export const getTasksMonth = async ({
  potId,
  year,
  month,
}: GetTasksMonthParams) => {
  return authApiGet<GetTasksMonthResponse[]>(`/my-pots/${potId}/tasks/month`, {
    year,
    month,
  });
};
...
export const getTasksCalendar = async ({
  ...
  return authApiGet<GetTasksCalendarResponse[]>(
    `/my-pots/${potId}/tasks/calendar`,
    { date }
  );
};
